<div class="container">

    <section id="welcome">

        <div class="row">

            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">

                <h1 class="text-center">Welcome To Wutsin Wheels</h1>

            </div>

        </div>

    </section>

    <section id="features">
        <div class="row">
            <h3 class="section-title featuredItemsHeader">Categories</h3>
            <hr>
            <div class="container">
                <!-- <div class="row"> -->
                    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                        <div class="thumbnail">
                            <a href="/ads?search=skateboard" class="thumbnail custom-anchor"> <!-- this link will go to the item page -->
                                <div class="caption">
                                    <img src="/img/hulk_skateboard.png" alt="Image File Path Bad" class="featuredImg">
                                    <h4 class="custom-h3">Skateboards</h4>
                                    <p>View Skateboard Ads</p>
                                </div>
                            </a>
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                        <div class="thumbnail">
                            <a href="/ads?search=bicycle" class="thumbnail custom-anchor"> <!-- this link will go to the item page -->
                                <div class="caption">
                                    <img src="/img/mars_attacks_bike.jpg" alt="Image File Path Bad" class="featuredImg">
                                    <h4 class="custom-h3">Bicycles</h4>
                                    <p>View Bicycle Ads</p>
                                </div>
                            </a>
                        </div>
                    </div>
                    <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
                        <div class="thumbnail">
                            <a href="/ads?search=unicycle" class="thumbnail custom-anchor"> <!-- this link will go to the item page -->
                                <div class="caption">
                                    <img src="/img/gizmoduck_uni.jpg" alt="Image File Path Bad" class="featuredImg">
                                    <h4 class="custom-h3">Unicycles</h4>
                                    <p>View Unicycle Ads</p>
                                </div>
                            </a>
                        </div>
                    </div>
                <!-- </div> -->
            </div>
        </div>
    </section>

</div>
